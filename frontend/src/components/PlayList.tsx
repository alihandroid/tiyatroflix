...
const PlayList: React.FC = () => {
  const { isLoading, error, data: plays } = useQuery<Play[]>({
    queryKey: ['plays'],
    queryFn: async () => {
          ...
          <Link to={`/plays/${play.id}`} key={play.id}>
            <Card className="hover:shadow-lg transition-shadow duration-300">
              ...
              <CardContent>
                ...
                <p className="text-sm text-gray-600 mb-2 font-semibold">Director: {play.director}</p>
                <p className="text-sm text-gray-500 line-clamp-3">{play.description}</p>
              </CardContent>
            </Card>
          </Link>
        ))}
